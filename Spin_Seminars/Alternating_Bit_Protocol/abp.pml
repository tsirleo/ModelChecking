...
active proctype P() {
  ...
  :: a?0,seq -> if
                :: (seq == sendSEQ) -> atomic { printf("P: recv=%d\n", seq);
                                                ...
                                                sendSEQ = !sendSEQ }
                :: else -> printf("P: old seq\n"); // ignore 
                fi
  :: a?1,seq -> printf("P: error\n");
...
active proctype Q() {
  ...
  do
  :: d?msg,0,seq -> if
                    :: (seq != recvSEQ) -> atomic { printf("Q: recv=%d\n", seq);
                                                    recvMSG = msg; 
                                                    recvSEQ = seq }
                                           Received: skip
                    :: else -> printf("Q: wrong\n")
...
ltl ReliableDataTransmission {
  (D_ALIVE && A_ALIVE) -> [](P@Sending -> <>(Q@Received && recvMSG == sendMSG))
}